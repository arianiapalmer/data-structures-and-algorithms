...
const addTwo = (arr) => {
  let newArr = arr.map(index => index +2);
  return newArr;
};
...
const containsAnd = (arr) => arr.filter(index => index.includes('and'));
...
const oddValues = (arr) => arr.filter(index => index%2);
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(index => forbiddenValues.filter(value =>value === index));
};
